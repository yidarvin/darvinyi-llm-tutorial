...
import SFTLossMasking from '@components/widgets/ch13/SFTLossMasking';
import ChatTemplateComparison from '@components/widgets/ch13/ChatTemplateComparison';
...
<Callout type="note">
**Part V transition.** The architectural exploration is over. Part IV (Chapters 11 and 12) closed with the two major alternatives to standard dense transformers, MoE and SSMs. Part V changes the question. Given *any* pre-trained architecture, what do you do to make it useful? SFT (this chapter), preference optimization (Chapter 14), PEFT (Chapter 15), and distillation (Chapter 16) are the four answers. Part V's chapters are more sequential than Part IV's: they build on each other. SFT feeds DPO; DPO is sometimes done with LoRA; distillation can wrap either.
</Callout>
...
def quality_score(example):
    ...
    score = 1.0
    if len(response) < 10:
        score *= 0.3           # too short to be a real answer
    if len(response) > 2000:
        score *= 0.3           # too long; often padded/repetitive
    if "as an ai" in response.lower():
        score *= 0.3           # meta-reference
...
print(f"Masked loss:   {masked:.3f}")    # 5.369
```

With seed 0, unmasked loss is 5.328 (averaged over all 12 positions) and masked loss is 5.369 (averaged over only the 6 response positions); these are different values, confirming the mask actually changes which tokens the gradient sees.
...
# formatted[202:215] == '3+3 equals 6.'
```

Two assistant turns produce two ranges. Each slice of `formatted` at those indices reproduces the assistant content exactly, which is what downstream tokenization needs to build the loss mask.
...
# Total tokens: 29, in loss: 8 (28%)
```

8 of 29 tokens are in loss: the 3 content tokens plus the closing `<|im_end|>` from each of the two assistant turns. Every system/user token and every turn-opening marker (`<|im_start|>` + role name) is 0.
