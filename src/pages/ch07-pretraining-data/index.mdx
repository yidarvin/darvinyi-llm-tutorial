...
Deduplication removes redundant content. Quality filtering removes low-value content, text that is unique but not worth training on. The two passes are complementary: dedup throws away the same content seen twice, quality filtering throws away content that no model should be learning from in the first place. Modern pipelines combine three flavors of filter, in roughly increasing order of cost and sophistication.
