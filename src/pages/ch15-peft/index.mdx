...
import LoRAArchitecture from '@components/widgets/ch15/LoRAArchitecture';
import ParameterBudgetCalculator from '@components/widgets/ch15/ParameterBudgetCalculator';
...
The arithmetic is uncomfortable. Weights plus gradients plus optimizer state alone is $140 + 140 + 560 = 840$ GB. An A100 has $80$ GB of HBM; an H100 also has $80$ GB. Full fine-tuning of a $70$B model needs at least $10$ A100s or H100s before you even budget for activations and communication buffers. FSDP from Chapter 9 helps by sharding parameters, gradients, and optimizer state across GPUs, but it does not reduce the *total* memory budget. You still need ten GPUs' worth of HBM, just split across the cluster. For a team with one or two GPUs, full fine-tuning of frontier-scale models is not an option you can engineer around.
...
The numpy version makes the mechanics explicit; production code wraps an existing `nn.Linear` instead of writing the forward pass out by hand. The pattern is a thin module that holds the frozen base layer plus its own trainable $A$ and $B$, and sums the two paths in `forward`, exactly as in <EqRef id="15.lora" />:

<RunnableCode client:visible runnable={false} readonly>
```python
import torch
...
class LoRALinear(nn.Module):
    ...
    def __init__(self, base, r=4, alpha=8):
        super().__init__()
        self.base = base
        self.base.weight.requires_grad_(False)   # freeze the base layer
...
# A tiny two-layer "model", both linear layers wrapped in LoRA
d_model = 64
base = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, d_model))
model = nn.Sequential(LoRALinear(base[0]), nn.ReLU(), LoRALinear(base[2]))
...
# The optimizer only ever sees the trainable adapter parameters
...
base_weight_before = base[0].weight.clone()
...
VeRA, Vector-based Random Matrix Adaptation (Kopiczko et al. 2023), pushes the parameter count down further by asking a different question: does every layer need its *own* $A$ and $B$? VeRA's answer is no. Freeze a single pair of random matrices $A_{\text{shared}}$, $B_{\text{shared}}$, sized to the largest layer and sliced as needed, and share them across *every* LoRA insertion point in the network. Neither matrix ever trains. What trains instead is a pair of small per-layer diagonal scaling matrices, $\Lambda_b$ and $\Lambda_d$, that rescale $B_{\text{shared}}$ and $A_{\text{shared}}$ row-by-row and column-by-column: $\Delta W = \Lambda_b B_{\text{shared}} \Lambda_d A_{\text{shared}}$.

Per layer, that is $r + d$ trainable scalars (the diagonal entries of $\Lambda_d \in \mathbb{R}^{r \times r}$ and $\Lambda_b \in \mathbb{R}^{d \times d}$) instead of LoRA's $r(d+k)$, roughly $10\times$ fewer parameters than LoRA at the same rank. The random matrices carry no task-specific information at all; they are fixed, shared, frozen scaffolding, and the entire adaptation lives in the diagonal scaling. It is a more extreme version of the low-rank hypothesis: not just "the update is low rank" but "the update is a rescaling of a fixed random subspace." VeRA trades away some expressivity for an order of magnitude fewer trainable parameters than even LoRA, which matters when you are storing thousands of small per-task adapters and every megabyte multiplies.
...
<Callout type="aside">
PEFT is an active research area in 2024 and 2025. New variants appear monthly. Stick with LoRA or QLoRA as your default. Explore DoRA when you want finer-grained updates at the same parameter budget; VeRA when you are storing many adapters and need the smallest possible footprint per task; AdaLoRA when you have a tight parameter budget and need to allocate it carefully; MoLoRA-style approaches when you have a clean multi-task setup. Don't optimize prematurely.
</Callout>
...
# 4. The trainable/base ratio DROPS for larger models (e.g. ~0.24% at 7B -> ~0.12% at
#    70B, r=16, Q+K+V+O), because the base grows quadratically in d_model while the
#    LoRA adapter grows only linearly in d_model
`}
  packages={["numpy"]}
/>
...
#   13B: 26.2M  0.202%
#   70B: 83.9M  0.120%
# -> ratio drops as model size grows, base scales faster than the adapter.
```
...
The starter code below uses the row-vector convention ($x$ is a row of shape `(batch, d_in)`, and `lora_forward_adapter` computes `x @ A.T @ B.T`). In that convention, the merged weight is the transpose of $BA$:
$$W_{\text{merged}} = W_0 + \frac{\alpha}{r} A^T B^T$$

Both forward modes should produce identical outputs (up to floating-point precision):
1. **Adapter mode**: $h = x W_0 + (\alpha/r) x A^T B^T$
2. **Merged mode**: $h = x W_{\text{merged}}$

Watch the orientation: because $d_{\text{in}} = d_{\text{out}} = 256$ here, a merge using $BA$ (instead of $A^T B^T$) would still produce a same-shape matrix, but it is the *wrong* matrix, it would silently fail to match adapter-mode instead of raising a shape error.
...
<Callout type="warning">
**Misconception.** "LoRA is a lossless compression of full fine-tuning, just cheaper." Biderman et al. (2024, "LoRA Learns Less and Forgets Less") measured this directly: across instruction tuning and continued pre-training on code and math, LoRA consistently underperforms full fine-tuning on the target task, especially on the hardest domains, while forgetting less of the base model's prior capability. LoRA is a genuinely different training regime, not a compressed rerun of full FT with the same endpoint. The gap is small for mild adaptation tasks and grows for the deep-capability-shift cases above; that gap is precisely why full FT still earns its keep some of the time.
</Callout>
...
Chapter 16, the final chapter of Part V, covers **distillation**: how to compress a fully trained model into a smaller one for deployment. Where Chapters 13–15 covered how to *train* aligned models, Chapter 16 covers how to *deploy* them efficiently when even an inference-optimized $70$B is too large. After Chapter 16, Part V closes and Part VI (inference and serving, Ch 17–19) begins.
