...
MMR sits downstream of reranking, not instead of it: rerank first for relevance, then diversify the top-K before handing it to the LLM. Skipping it costs nothing on corpora with little redundancy; on corpora with lots of near-duplicate content, restated FAQ answers, template policy variations, MMR is the difference between a top-K that says one thing five times and a top-K that actually covers the question.
