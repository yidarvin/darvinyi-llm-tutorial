...
Before 2013, learned word representations existed (distributional models going back to LSA and HAL, neural language models like Bengio et al. 2003) but they were expensive and unstable at the scale of web text. Mikolov, Chen, Corrado, and Dean (2013, [arxiv.org/abs/1301.3781](https://arxiv.org/abs/1301.3781)) changed this with a simple, scalable, self-supervised setup: word2vec.
...
<Callout type="aside">
The parameter savings from tying are dramatic at small scale (~30% of a GPT-2-small) and diminish at large scale. For a 7B-parameter model, tied versus untied is a ~1% parameter difference, barely visible against the FFN and attention blocks that dominate. Tying remains the default at all scales mostly because it costs nothing and occasionally helps perplexity by a small margin (Press and Wolf reported ~5%). The marginal gain is empirical and not always reproducible; the parameter savings are mechanical and always real.
</Callout>
