...
The empirical evidence for this gap is unambiguous and it was the original motivation for the field. **GPT-3 on GSM8K** (the canonical grade-school math benchmark, free-response, not multiple-choice) scores about $16\%$ with direct generation. With chain-of-thought prompting, GPT-3-class models reach roughly $46\%$, roughly a threefold improvement from nothing more than a change in prompt structure. Larger models show the same pattern with a higher ceiling: PaLM 540B goes from $17\%$ direct to $57\%$ with CoT. The model already had the reasoning capability latent in its weights; what was missing was the inference-time machinery to elicit it.
