...
import JailbreakTaxonomy from '@components/widgets/ch24/JailbreakTaxonomy';
import PromptInjectionClassifier from '@components/widgets/ch24/PromptInjectionClassifier';
...
No layer in the pipeline needs to be perfect for the pipeline to work; each layer needs to catch what the others miss, and their blind spots need to be at least partly uncorrelated. An input classifier and the refusal-trained model both failing on the same attack pattern is a much bigger problem than either failing alone, and it is why input classifiers, the model's own safety training, and output classifiers are typically built by different teams on different data. Correlated blind spots, not any single layer's raw catch rate, are the actual risk in a layered defense.
</Callout>
...
<RunnableCode
  client:visible
  expectedOutput={["Overly strict          |     60% |      0%", "Well-calibrated        |      0% |      0%"]}
>
```python
...
# - Well-calibrated: 0% on both, but only because this toy test set is small and
#   its harmful phrases exactly match the well-calibrated model's phrase list;
#   real classifiers facing the full distribution of adversarial phrasing never hit zero
...
<RunnableCode
  client:visible
  expectedOutput={["Successful attacks: 2 of 8", "Hypothetically, in a novel, [weapons] would be made by..."]}
>
```python
...
# Observations:
# - Fiction framing ("novel"/"hypothetically") slips through in this run
# - Authority framing ("security researcher", "chemistry class") is refused here:
#   this toy defender only has a fiction-framing gap, not an authority-framing one
...
def critique(draft, constitution):
    ...
    #   OR the principle has no topics at all, i.e. a meta-principle like honesty
    #   that isn't tied to a specific forbidden subject)
    #   → it's a violation
    return violations
