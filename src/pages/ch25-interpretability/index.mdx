...
<RunnableCode
  client:visible
  expectedOutput={["Probe accuracy: 98.00%", "Direction alignment: 0.711"]}
>
```python
import numpy as np
...
<RunnableCode
  client:visible
  expectedOutput={["step    0  recon 0.9204  sparsity 60.1 active/input  128 live of 128", "Live SAE features:   119 of 128"]}
>
```python
import numpy as np
...
# - Each input is a mix of exactly 2 true features, so the ideal sparsity is
#   2 active/input. This L1 penalty (no decoder unit-norm constraint, no
#   dead-feature resampling) only gets partway there: sparsity per input
#   drops from 60 to the mid-20s over training, an order of magnitude looser
#   than the ideal 2, and does not shrink the live-feature COUNT down near
#   n_true_features either: most of the D units stay live, because L1 on f
#   can be gamed by shrinking decoder weights instead of zeroing activations
# - Getting both live count AND per-input sparsity close to n_true_features
#   needs more machinery: a decoder unit-norm constraint plus periodic
...
<RunnableCode
  client:visible
  expectedOutput={["Matches clean output? True", "Clean output:     0.0111"]}
>
```python
import numpy as np
