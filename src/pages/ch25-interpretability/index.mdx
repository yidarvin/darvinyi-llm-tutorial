...
import SAEFeatureExplorer from '@components/widgets/ch25/SAEFeatureExplorer';
import LinearProbingVisualizer from '@components/widgets/ch25/LinearProbingVisualizer';
...
# Observations:
# - The probe direction is only moderately aligned with the planted direction
#   (~0.7 cosine, not 1.0): high accuracy does not require finding the exact
#   direction, just a decision boundary that separates the classes well
...
live_final = (f > 0.01).any(axis=0).sum()
print(f"\nTrue feature count:  {n_true_features}")
print(f"Live SAE features:   {live_final} of {D}")

# Observations:
# - The SAE is wider than the input (D > d): room to un-pack superposition
# - The L1 penalty forces few features active PER INPUT (~2-3), but that alone
#   does not shrink the live-feature COUNT down near n_true_features: most of
#   the D units stay live, because L1 on f can be gamed by shrinking decoder
#   weights (no unit-norm constraint here) and dead units are never resampled
# - Getting live count close to n_true_features needs more machinery: a
#   decoder unit-norm constraint plus periodic dead-feature resampling
# - In real SAEs at production scale: millions of features, mostly
#   monosemantic, achieved with exactly these additional tricks
```
</RunnableCode>
...
**Logit lens** (nostalgebraist $2020$), project intermediate residual stream activations through the unembedding matrix; see what the model "would predict" at each layer. Surfaces the layer-by-layer evolution of predictions. An observational read-out, not an intervention, and a useful baseline before reaching for the four techniques below.

**Tuned lens** (Belrose $2023$), a refined logit lens with learned layer-specific projections. More accurate than the unmodified logit lens; the same observational frame, still not causal.
...
# # - L1 sparsity alone tends to leave far more than n_true_features live (see
# #   the chapter's runnable SAE demo); hitting the true count needs more than
# #   raising lambda, e.g. a decoder unit-norm constraint and dead-feature resampling
...
2. Each of the network's 4 components writes additively into a shared output, like heads in a residual stream. For patch depth $L$, use the clean run's components for depths $0..L$ and the corrupted run's components for the rest.
3. Measure how much the output flips toward the clean answer as $L$ grows.
...
# A toy 4-component network: each component reads the input directly and
# writes additively into a shared output, the way attention heads/MLPs at
# different depths all write into a transformer's residual stream. (A
# strictly sequential chain would make every patch point recover 100% of
# the answer, since re-running the rest of a deterministic chain from a
# clean activation always reproduces the clean output; additive components
# let us patch one depth's contribution while the others stay corrupted.)
np.random.seed(0)

n_layers = 4
d = 8

# Random weights per component. Component 0 is built to barely depend on
# the one input dimension that differs between clean and corrupted prompts;
# later components depend on it increasingly strongly.
W = [np.random.randn(d, d) * 0.5 for _ in range(n_layers)]
dim0_direction = np.random.randn(d) * 0.5
listens_to_dim0 = [0.05, 0.9, 0.95, 1.0]
for L in range(n_layers):
    W[L][0, :] = dim0_direction * listens_to_dim0[L]

readout = np.array([0.5, -0.3, 0.4, -0.1, 0.2, 0.1, -0.2, 0.3])


def component_outputs(x):
    """Each component's contribution, computed independently from the input."""
    return [np.tanh(x @ W[L]) for L in range(n_layers)]


def model_output(contribs):
    """Sum the components into a shared stream, then read out."""
    return sum(contribs) @ readout


def patched_forward(clean_contribs, corrupt_contribs, patch_depth):
    """
    Use the clean run's components for depths 0..patch_depth, and the
    corrupted run's components for everything after. Returns the output.
    """
    # TODO:
    # 1. Build a list where entries 0..patch_depth come from clean_contribs
    #    and entries after patch_depth come from corrupt_contribs
    # 2. Return model_output(...) on that mixed list
    pass


# Two prompts: clean vs corrupted (differ only in input dim 0)
...
clean_contribs = component_outputs(clean_input)
corrupt_contribs = component_outputs(corrupt_input)
out_clean = model_output(clean_contribs)
out_corrupt = model_output(corrupt_contribs)

print(f"Clean output:     {out_clean:.4f}")
print(f"Corrupted output: {out_corrupt:.4f}")
print(f"Gap to recover:   {out_clean - out_corrupt:.4f}\\n")

# Patch cumulatively through depth 0, 0-1, 0-2, 0-3; report recovery
...
#
# # Observations:
# # - Recovery climbs sharply once depth 1 is patched, then approaches 100% by depth 3
# # - Depth 0 barely matters here: patching it alone recovers almost nothing
# # - The earliest depth where recovery jumps is where the info lives
...
def label_feature(feature):
    ...
    #    - if common French function words (vous, un, en) appear → "French language"
    #      (note: the tokenizer above strips accents, so match on unaccented
    #      function words, not accented characters)
    #    - if "can't" or "won't" or "decline" → "Refusal patterns"
