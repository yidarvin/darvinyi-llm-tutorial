...
import TemperatureScaling from '@components/widgets/ch16/TemperatureScaling';
import DistillationPipeline from '@components/widgets/ch16/DistillationPipeline';
...
A $70$B model in BF16 occupies roughly $140$ GB just for the weights. Serving it requires at least $2 \times$ A100 $80$GB per inference instance; at production batch sizes, throughput lands around $50$ tokens per second per instance and per-token decode latency near $25$ ms. A $7$B model in the same precision fits on a single A100, or even a single A6000, pushes around $500$ tokens per second, and decodes at roughly $5$ ms per token. That is an order of magnitude cheaper per request, plus much higher concurrency per GPU because batches fit more sequences. For a product surface measured in queries per second per dollar, the gap is decisive.
...
<RunnableCode
  client:visible
  expectedOutput={["T=  1 | class 0: 0.554", "top class dominates (~0.55)"]}
>
```python
import numpy as np
...
print("\nObservations:")
print("- T=1:  top class dominates (~0.55); dark knowledge hidden")
...
<RunnableCode
  client:visible
  expectedOutput={["T=  1 |   2.126   2.693   1.883", "Hard loss is independent of T"]}
>
```python
import numpy as np
...
def softmax_rows(scores):
    """Row-wise softmax: turns raw attention scores into attention probabilities."""
    scores = scores - scores.max(axis=-1, keepdims=True)
    exp = np.exp(scores)
    return exp / exp.sum(axis=-1, keepdims=True)

def attention_distillation_loss(student_scores, teacher_scores):
    """
    Attention distillation loss (TinyBERT-style): MSE between attention
    probability maps, averaged over heads and mapped layers.

    student_scores, teacher_scores: (layers, heads, seq, seq) raw attention
    logits (pre-softmax). Layers must already be paired via a layer-mapping
    strategy (e.g. student layer l <-> teacher layer 3l) and head counts
    must match between the paired layers.
    """
    p_teacher = softmax_rows(teacher_scores)
    p_student = softmax_rows(student_scores)
    return ((p_teacher - p_student) ** 2).mean()

# Toy setup: 2 mapped layers, 4 heads, sequence length 6
np.random.seed(0)
layers, heads, seq = 2, 4, 6

teacher_scores = np.random.normal(0, 2, (layers, heads, seq, seq))
# A randomly initialized student, attention maps unrelated to the teacher's
student_scores_random = np.random.normal(0, 2, (layers, heads, seq, seq))
# A "trained" student whose attention scores have moved close to the teacher's
student_scores_trained = teacher_scores + np.random.normal(0, 0.1, (layers, heads, seq, seq))

loss_random = attention_distillation_loss(student_scores_random, teacher_scores)
loss_trained = attention_distillation_loss(student_scores_trained, teacher_scores)

print(f"Attention MSE, random-init student: {loss_random:.5f}")
print(f"Attention MSE, distilled student:   {loss_trained:.5f}")
print("\nThe distilled student's attention maps track the teacher's row-by-row;")
print("the random student's don't. This auxiliary loss is added to the response")
print("distillation loss during training; it never appears alone in practice.")
```
</RunnableCode>
...
<Callout type="note">
R1-Distill is the most exciting recent demonstration that **reasoning capabilities can be distilled**. The student does not need to discover chain-of-thought from scratch via RLVR, a process that is genuinely expensive and difficult to stabilize. It can inherit the capability from a teacher that already has it. This dramatically lowers the cost of producing reasoning-capable models and is one of the most important post-training developments of the R1/o1 generation. The caveats are real: the student still does not match the teacher on the hardest problems, and very-long-tail capabilities may not transfer perfectly. But the direction is genuinely new.
</Callout>
...
<Callout type="warning">
**Misconception.** "Student capacity is basically irrelevant, distillation transfers whatever data you give it." False. **Student capacity bounds what can be transferred.** A $1.5$B model given the exact same reasoning traces as a $32$B model does not reach the $32$B's performance; it reaches whatever its own capacity allows it to absorb from those traces. Better teachers and more data narrow the gap. They do not eliminate the capacity floor.
</Callout>
...
# Verify:
# 1. At T=1, the top class should dominate (~0.83)
...
Divide by $T$ before the usual stable softmax; everything else is unchanged.
...
Output: `T=1: top prob = 0.833`, `T=4: top prob = 0.237`, `T=16: top prob = 0.126`, each row summing to `1.000`. The top probability falls from 0.83 toward 1/10 = 0.1 as $T$ grows, confirming the sharpen-at-low-$T$/flatten-at-high-$T$ behavior.
...
Sum $p_i (\log p_i - \log q_i)$ along the last axis, with an epsilon guard inside each log.
...
Soften both distributions at $T$, take their KL, scale by $T^2$ only when `use_T2` is set, and combine with the T=1 hard-label cross-entropy.
...
At $T=1$ the multiplier is a no-op ($1^2 = 1$), so the two columns match. By $T=16$, the no-$T^2$ soft loss has collapsed to 0.01 (softened distributions are close to uniform, so KL is tiny) while the $T^2$-scaled version stays around 2.6, confirming the multiplier is what keeps the soft-loss gradient from vanishing at high temperature.
