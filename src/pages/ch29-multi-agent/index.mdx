...
**Four. Parallelism is genuinely useful.** Subtasks that can truly run in parallel, research multiple topics, review multiple PRs. Speed gains from concurrent execution that justify orchestration overhead. Anthropic's own multi-agent research system ([anthropic.com/news/multi-agent-research-system](https://www.anthropic.com/news/multi-agent-research-system), $2025$) is a shipped production example of exactly this case, an orchestrator dispatching independent research subtasks to parallel subagents. It doesn't rebut the single-agent default; it's a worked example of the $20\%$ where multi-agent earns its place.
