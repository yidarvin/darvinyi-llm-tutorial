...
import MoERoutingVisualizer from '@components/widgets/ch11/MoERoutingVisualizer';
import ActiveVsTotalParams from '@components/widgets/ch11/ActiveVsTotalParams';
...
<Callout type="note">
**Part III to Part IV.** Part III (Chapters 7–10) was a contiguous training arc: data, then loop, then scaling, then infrastructure. Part IV is thematic instead. Chapters 11 (this one) and 12 (state-space models and Mamba) cover the two major architectural alternatives to the standard dense transformer that have emerged in the last few years. Both chapters are largely independent; readers can pick based on interest. After Part IV, the tutorial pivots into post-training (Chapters 13–16), a different texture again.
</Callout>
...
where $\sigma$ is a nonlinearity (GeLU in most production transformers; the runnable demo below uses ReLU for simplicity). One pass through, all parameters used.
...
$$\text{MoE}(x) = \sum_{i \in \text{top}_k(\ell)} g_i(x) \cdot \text{FFN}_i(x), \quad g_i(x) = \frac{\exp(\ell_i)}{\sum_{j \in \text{top}_k(\ell)} \exp(\ell_j)}$$
</Equation>
...
k = 2
output, indices, gates = moe_forward(x, router_W, expert_W1s, expert_W2s, k=k)
...
print(f"\nSparsity: {k}/{num_experts} = {k/num_experts:.0%} "
...
**Active parameters per token** = $k$ experts $\times$ per-expert FFN size, plus the shared attention/embedding/output/router parameters. For Mixtral: about $12.9$B. The other $5.6$B $\times 6 \approx 33.6$B of expert parameters are loaded in memory but not invoked for that token.
...
DeepSeek-V3 took the DeepSeek-MoE recipe to the frontier: $671$B total parameters, $37$B active per token. It was, as of late $2024$, the open-weights state of the art on most benchmarks. Notably, it dispenses with the explicit auxiliary loss in favor of **auxiliary-loss-free load balancing**, a bias-adjustment scheme that nudges router logits toward balance without adding a separate loss term. Reports indicate it works better than the classic Switch Transformer auxiliary loss, suggesting that the load-balancing problem is genuinely still being refined.

The frontier kept moving on exactly this recipe. DeepSeek itself shipped **DeepSeek-V4** in April $2026$ (open weights, MIT-licensed): a V4-Pro variant at $1.6$T total / $49$B active parameters and a smaller V4-Flash at $284$B total / $13$B active, both with a $1$M-token context window, continuing the fine-grained-expert, loss-free-balancing template set by V3. By mid-$2026$, independent benchmarks (Artificial Analysis's Intelligence Index) put Z.ai's GLM-5.2 ($753$B total, about $40$B active, MIT-licensed, released June $2026$) ahead of DeepSeek-V4-Pro, with Moonshot's Kimi K2.6 (roughly $1$T total, $32$B active) also in the same tier. As of $2026$-$07$-$13$, "the open-weights frontier" names a cluster of Chinese-lab MoE releases rather than any single model, and whichever one leads will likely change again within months; DeepSeek-V3 remains the right teaching example for the loss-free-balancing technique, but it is a $2024$ snapshot, not the current state of the art.
...
print("→ Mixtral uses ~13B params per token; Llama-2 70B uses 70B.")
print("  Mixtral has lower per-token inference cost despite more total capacity.")
print("  (These are simplified estimates: real embeddings, tied weights, and Llama-2's")
print("   grouped-query attention are not modeled here, so exact published figures differ.)")
```
</RunnableCode>
...
The progression from Switch (top-1, $2021$) to GLaM (top-2, $2021$) to Mixtral (top-2 open-weights, $2024$) to DeepSeek-V3 (loss-free balancing, late $2024$) is a clean four-step story. Each step solved a different practical problem: Switch made sparse scaling work at all, GLaM established top-2 as standard, Mixtral made MoE production-ready in open weights, DeepSeek-V3 pushed it to frontier scale with refined balancing. What remains is the honest accounting of why MoE is harder to use than dense models.
...
Post-training MoE models is harder than post-training dense models for a structural reason: each expert sees roughly $k/N$ of the data. A fine-tuning corpus of $10$K examples becomes effectively $\sim 2500$ examples per expert in an $8$-expert model with top-2 routing. Gradient signal per expert is thinner, training is noisier, and overfitting risk is higher.
...
Route by the top-$k$ logits, softmax only over those (not the full vector), and combine each selected expert's FFN output weighted by its gate:
...
- Per-expert FFN (SwiGLU: gate, up, down projections): $3 d \cdot d_{\text{ffn}}$
- Router: $N \cdot d$
...
<RunnableCode
  ...
  defaultCode={`def count_moe_params(num_layers, d_model, d_ffn, num_experts, top_k, vocab_size=32000):
    ...
    ln = 4 * d_model
    # FFN per expert (SwiGLU: gate + up + down projections): 3 * d_model * d_ffn
    ffn_per_expert = 3 * d_model * d_ffn
...
# print(f"\\n-> Mixtral has ~61% the total params and ~17% the active params of Llama-2 70B.")
# print(f"   Same per-token compute as Llama-2 13B (active ~13B); memory footprint of a ~47B model; quality closer to Llama-2 70B.")
`}
  packages={[]}
/>
...
Total counts every expert's FFN; active counts only the top-$k$ FFNs actually run per token. Everything else (attention, layernorms, router, embeddings) is dense and counted once either way:
...
$f_i$ is a hard count (from argmax assignments), and $P_i$ is a soft average (from the full softmax); mixing a hard and soft signal is what makes this loss differentiable despite argmax being involved:
...
With the seeded setup: balanced routing loss = 0.01001 (close to the theoretical minimum $\alpha \cdot N \cdot N \cdot (1/N)^2 = \alpha = 0.01$), collapsed routing loss = 0.03841, a 3.84x ratio: collapse is heavily penalized.
...
# At rho=1.0: imperfect routing -> ~29% of tokens dropped
# At rho=1.25: drop rate roughly a third lower (~20%); some capacity wasted
# At rho=2.0: drop rate down to ~7% -- this weight skew is severe enough that
#   even generous capacity doesn't reach zero drops; real router distributions
#   trained with an aux loss are far less skewed than this synthetic example
...
Even at $\rho = 2.0$ drops don't hit zero, because this synthetic skew is severe (the busiest expert gets 2.5x the fair share); real trained routers, kept balanced by the auxiliary loss, don't skew this hard.
