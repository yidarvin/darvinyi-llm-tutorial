...
Attention has a problem. It is $O(N^2)$ in both memory and compute. At a $32$K-token context, the attention matrix has a billion entries, about $4$GB in BF16 just to hold the scores for a single layer. At the rumored million-token frontier, the same matrix is a trillion entries, four terabytes. FlashAttention (Chapter 10) hid this cost from HBM by keeping intermediate tiles in SRAM, but the underlying compute is still quadratic: FlashAttention is a better implementation of the same $O(N^2)$ algorithm, not a different one. Long contexts are bottlenecked by attention, full stop.
...
## The setup: attention's $O(N^2)$ problem
...
FlashAttention (Chapter 10) reduced the *I/O cost* from $O(N^2)$ HBM reads to $O(N)$ HBM reads by tiling the softmax through SRAM. That single optimization made $32$K-token training practical and is responsible for most of the long-context capability frontier models advertise today. But the underlying *compute* is unchanged: the algorithm is still summing $N^2$ pairwise scores, just doing it more efficiently. **FlashAttention hides the cost; it does not eliminate it.** Past $128$K or so, even FlashAttention's accelerated quadratic is too expensive to be practical at scale.

What we want is **linear-time sequence modeling**. Memory and compute both growing as $O(N)$ in the sequence length, not $O(N^2)$. Constant memory per token at inference, ideally, no growing KV cache, no quadratically growing attention matrix. Several research lines have proposed such alternatives:
...
The bookkeeping trade-off is clean. Attention's KV cache is exact and unbounded; SSMs' state is compressed and bounded. Attention can recall any past token with full fidelity; SSMs cannot. Attention pays $O(N)$ memory per layer per token to store the cache; SSMs pay $O(d_{\text{state}})$ regardless of context length. For long contexts the SSM's bounded state is the entire architectural advantage. For tasks requiring needle-in-a-haystack retrieval, it is the architecture's central limitation.
...
The continuous formulation is mathematically clean but does not directly run on a computer, text tokens are discrete, not a continuous-time signal. The next section covers how the continuous system becomes a discrete recurrence.
...
The recurrence is one way to compute the SSM. The next section shows the other, a parallel mode that the linear structure unlocks for free.
...
- **Recurrence mode**: $h_t = \bar{A} h_{t-1} + \bar{B} x_t$, then $y_t = C h_t$. $O(N \cdot d_{\text{state}})$ time per token, **sequential**, token $t$ depends on token $t-1$. Constant memory per token (just the state). Excellent for inference, one token at a time.
...
The duality is mathematically beautiful but has a limitation that becomes the central plot point of the next section. **It only works for time-invariant systems**, systems where $\bar{A}, \bar{B}, \bar{C}$ do not change across time steps. The moment we make the parameters depend on the input, the convolution mode breaks. Section 5 explains why Mamba does that anyway.
...
Mamba (Gu and Dao, December $2023$) is one architectural change away from S4. **Make the parameters depend on the input.** Where S4 has fixed $\bar{B}, \bar{C}, \Delta$, Mamba computes them per token from $x_t$:

$$\bar{B}_t = \text{Linear}_B(x_t), \quad \bar{C}_t = \text{Linear}_C(x_t), \quad \Delta_t = \text{softplus}(\text{Linear}_\Delta(x_t))$$

(The matrix $A$ itself is kept fixed; what varies per token is $\bar{A}_t = \exp(\Delta_t A)$ via $\Delta_t$.)
...
This is a deliberate trade-off. Mamba gives up the convolution mode to gain selectivity. The duality, S4's signature mathematical elegance, is sacrificed for a quality gain that lets SSMs finally compete with transformers on language. It is not a flaw of Mamba; it is a chosen complexity. And it sets up the engineering problem that the next section solves: how to compute a time-varying linear recurrence efficiently on a GPU.
...
Breaking the convolution mode means we need a new algorithm to compute Mamba's recurrence efficiently. Section 6 covers it.
...
This is the **selective scan**. The name "selective" reflects Mamba's input-dependent parameters; the "scan" is Blelloch's parallel scan applied to those time-varying operators. **Selective scan is the algorithm.** It exists because Mamba's selectivity broke the convolution mode and made the naive recurrence unusable on GPUs.
...
- **Fused input/output projections.** The Linear layers that produce $\bar{B}_t, \bar{C}_t, \Delta_t$ and the final output projection are folded into the same CUDA kernel as the scan itself, eliminating intermediate memory writes.
...
The engineering pattern is identical to FlashAttention. **The algorithm was largely known**, parallel scans on associative operators are textbook computer science. **The kernel made it practical.** This is the connection back to Chapter 10's "hardware-aware implementation" thesis: at the frontier, custom CUDA kernels are not optimization; they are the difference between an algorithm working and not working.
...
With the math and the algorithm in place, the next section steps back from the SSM layer in isolation and looks at how it composes into a full architecture.
...
**Hybrid models often win.** **Jamba** (Lieber et al., $2024$) is the canonical example: a frontier-scale model that interleaves Mamba and transformer layers, with MoE FFNs on top of the transformer layers. The reasoning is empirical. Attention is genuinely better for some operations, in-context learning, retrieval, exact recall of demonstrations. Mamba is genuinely better for most operations, efficiency, long-context scaling, throughput. **Hybrid captures both.** The current common pattern is roughly $80\%$ Mamba layers and $20\%$ transformer layers, distributed throughout the network so that attention is available periodically without dominating compute.
...
**Short contexts (under $\sim 8$K).** Attention's optimized matmul kernels (FlashAttention, paged attention) are faster in absolute terms than SSM kernels at typical transformer-serving sequence lengths. The crossover where SSMs win on wall-clock is meaningfully past where most production transformers operate. For chatbots and code assistants with contexts under $8$K, attention wins on raw speed.
