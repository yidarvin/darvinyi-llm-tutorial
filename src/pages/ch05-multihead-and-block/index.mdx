...
GELU has a closed-form definition through the normal CDF, but the formula has no elementary form, so implementations use a tanh-based approximation that is accurate to about three decimal places:
...
The argument for the simplification is that the mean subtraction in standard layer norm is not load-bearing — most of the benefit comes from the variance normalization, and skipping the mean saves a few operations per token. Empirically RMSNorm performs comparably to layer norm and runs slightly faster. The chapter implements full layer norm because it is the more general operation; RMSNorm is a special case worth knowing about, but the conceptual content is the same.
...
<WidgetFrame title="Pre-LN transformer block" caption="Data flowing through one Pre-LN transformer block: input → LN → attention → residual → LN → FFN → residual → output. Each sublayer adds a refinement; the residual path carries the bulk of the signal through.">
  <div style={{ aspectRatio: '16 / 9', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-tertiary)', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.875rem' }}>
