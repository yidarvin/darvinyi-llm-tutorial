...
import KVCacheAnimation from '@components/widgets/ch17/KVCacheAnimation';
import SpeculativeDecoding from '@components/widgets/ch17/SpeculativeDecoding';
...
<RunnableCode client:visible expectedOutput={["In production: the speedup is much larger because real attention is O(N^2) per layer.", "For 1024-token sequences, KV cache provides ~700× speedup vs naive."]}>
```python
import numpy as np
import time
...
<RunnableCode client:visible expectedOutput={["k=  5 |  1.24x  1.71x  2.56x  4.07x", "High acceptance (α=0.7-0.9): k=5-7 gives 2-3x speedup"]}>
```python
