...
import AgenticLoopVisualizer from '@components/widgets/ch27-agent-foundations/AgenticLoopVisualizer';
import AgentPatternCatalog from '@components/widgets/ch27-agent-foundations/AgentPatternCatalog';
...
The chapter is engineering with honest limits. Agents work well for narrow, well-defined tasks: GAIA scores at $60$ – $75\%$ for frontier agents; SWE-bench Verified at ~$50\%$; meaningful production deployments. Open-ended autonomous agents remain unsolved, the $2023$ framing has given way to $2025$'s bounded-autonomy framing. By the end of this chapter, you'll have the conceptual toolkit and the historical context. Then Ch 28 builds agents from scratch; Ch 29 composes multiple agents; Ch 30 evaluates them and closes the curriculum.
...
The empirical scale in early $2025$ is worth keeping in mind. **GAIA**: frontier agents $60$ – $75\%$ on multi-step real-world tasks (humans about $92\%$). **SWE-bench Verified**: frontier agents around $50\%$ on real GitHub issues. Production deployments include Anthropic Claude Code, GitHub Copilot Workspace, Cursor, Devin, Aider, and OpenAI's o-series with tool use. In the open-source ecosystem, LangChain sits at roughly $100{,}000$ GitHub stars and AutoGPT at roughly $170{,}000$.
...
<Callout type="aside">
*"An agent's autonomy is the goal"* is false in production. Bounded autonomy is the production goal: agents should be as autonomous as the task allows, but no more. Human-in-the-loop checkpoints, explicit scope limits, and oversight mechanisms are the difference between a useful agent and a runaway system. $2025$'s production agents emphasize bounded autonomy because that is the configuration that ships.
</Callout>
...
Running `react_agent("What's the weather in Tokyo, and what date is it today?")` traces three iterations: `weather("Tokyo")` -> `"18°C, partly cloudy in Tokyo"`, `date()` -> `"2025-05-22"`, then `final_answer(...)`, returning `"Today (May 22, 2025) in Tokyo: 18°C, partly cloudy."` Every fact in the answer came from an observed tool call, not a fabrication.
...
`reflexion_run(...)` then succeeds on attempt 3: attempt 1 fails and adds reflection 1, attempt 2 fails (sees 1 reflection) and adds reflection 2, attempt 3 sees 2 reflections and `attempt()` returns `("Solution to '...'", True)`.
...
def sketch_react_for_task_2():
    ...
    return f"Bhutan's population density is approximately {density:.2f} people per square kilometer."
```

All three tasks classify correctly: task_1 -> `linear` (no chain, a lookup verb), task_2 -> `react` (contains "then"), task_3 -> `reflexion` (contains "fix" and "test"). `sketch_react_for_task_2()` returns "Bhutan's population density is approximately 20.50 people per square kilometer." (`787000 / 38394 = 20.4979944783039`, rounded to `20.50`.)
