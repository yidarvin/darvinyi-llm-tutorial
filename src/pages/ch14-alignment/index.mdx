...
<Callout type="note">
**Four families, one frame.** A token-level MDP turns every later method into an answer to two questions: *how do you weight each token, and how do you keep the model near the reference?* The chapter then walks four answers in turn, classical **RLHF** (reward model + PPO with KL folded into the reward), **DPO** (the closed-form offline collapse), **GRPO** (critic-free, with a group-relative Monte Carlo baseline), and **RLVR** (a verifier as reward), and synthesizes them near the end as instances of one reweighted-cross-entropy pattern. The final sections cover practical pathologies and the production stack.
</Callout>
