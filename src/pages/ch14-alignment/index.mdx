...
The rest of the chapter walks the four answers in order. The next section starts with the data (preferences and the Bradley-Terry reward model) before turning to the first answer (PPO).
...
With a reward model in hand, the next question is how to *use* it. Section 4 covers the classical answer: PPO.
...
In words: the policy *wants* high reward but is *penalized* for drifting from the reference policy $\pi_{\text{ref}}$, typically $\pi_{\text{SFT}}$, the frozen SFT model. The temperature $\beta$ controls the trade-off. Small $\beta$ lets the policy chase reward aggressively, accepting large KL; large $\beta$ keeps the policy close to the SFT model, accepting smaller reward gains. Section 5 zooms in on the KL constraint specifically; for now, take it as a regularizer.
...
Section 5 zooms in on the KL constraint, what it does, why it matters, and what happens without it.
...
The DPO formulation has spawned a family of related methods. Section 7 covers four of the most cited.
...
The next two sections return to RL with two changes layered on PPO. Section 8 introduces **GRPO**, which keeps PPO's clipped surrogate but drops the value function: sample a group of completions per prompt and use the group's mean reward as the baseline directly. Section 9 then changes the reward source itself, replacing the learned reward model with a deterministic verifier (**RLVR**). The combination of those two changes, GRPO plus RLVR, is what DeepSeek used to train R1.
...
That property is what makes GRPO the natural partner for verifiable rewards in the next section. When the reward function is a binary correctness check, the group splits cleanly into "the few that worked" and "the many that did not," and the policy is pushed toward whatever reasoning trace the working completions used. The next section turns to that reward design choice.
...
Section 10 synthesizes the four methods (PPO, DPO, GRPO, RLVR) into the single token-level pattern previewed in section 2; section 11 then returns to the practical pathologies that show up in any preference-optimization setup, RLHF, DPO, GRPO, RLVR, and the standard mitigations.
...
The next section catalogues the practical pathologies that any of these methods can fall into, and the standard mitigations.
...
Section 12 closes the chapter by placing this material in the modern post-training stack.
