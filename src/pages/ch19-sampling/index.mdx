...
import SamplingDistribution from '@components/widgets/ch19/SamplingDistribution';
import ConstrainedDecoding from '@components/widgets/ch19/ConstrainedDecoding';
...
And then Part VI is complete. Three chapters covered the full inference-engineering arc: cache + batching + Flash Attention + speculative + paging (Ch 17); quantization + NF4 + GPTQ + AWQ (Ch 18); and sampling (this chapter). Combined, these are the difference between a $50$-token-per-second research curiosity and a $500$-to-$1{,}000$-token-per-second production service. Part VII picks up with capabilities, reasoning, tools, retrieval, multimodal, the chapters that turn deployable models into useful systems.

## From logits to text

### The three steps

Every decode step inside an LLM has the same three-step shape. **Logits** come out of the forward pass: a vector $z \in \mathbb{R}^V$ where $V$ is the vocabulary size, typically $50{,}000$ to $200{,}000$ entries. **Softmax** turns that into a probability distribution: $p_i = \exp(z_i) / \sum_j \exp(z_j)$, with the standard max-subtraction trick for numerical stability (see Chapter 4's derivation of scaled dot-product attention, where the same trick stabilizes the attention softmax). **Sampling** is the algorithm that picks a single token index $i^*$ from the distribution, and that index is the next emitted token. The forward pass is the same regardless of strategy; the sampler is what changes, and that third step is what this chapter is about.
...
<Callout type="aside">
Sampling is the highest leverage-to-complexity ratio in Part VI. A few lines of code (top-p, temperature, a mild repetition penalty) transform output quality. Compare to Ch 17 (sophisticated kernel engineering across days of work) and Ch 18 (intricate quantization recipes with calibration steps). Sampling is *not* where the engineering effort is hardest, but it *is* where small changes have the biggest user-facing effect. Tune carefully and the model behaves like a different product.
</Callout>
...
The interactive widget later in this chapter lets you dial $k$ directly, alongside top-p and temperature, and watch the kept-token set change in real time.
...
def top_k_sample(logits, k=50, T=1.0, rng=None):
    ...
    z = logits / T
    k = min(k, len(z))
    top_k_idx = np.argsort(z)[::-1][:k]
    mask = np.full_like(z, -np.inf)
    mask[top_k_idx] = z[top_k_idx]
    p = softmax(mask)
    return int(rng.choice(len(p), p=p))

def mass_captured(logits, k):
    """Diagnostic: how much probability mass falls inside the top-k set."""
    probs = softmax(logits)
    return np.sort(probs)[::-1][:k].sum()

def make_distribution(shape, V=200):
    np.random.seed(0)
...
print(f"{'Shape':<10} | {'mass, k=10':>10} | {'mass, k=50':>10} | {'mass, k=100':>11}")
print("-" * 48)
for shape in ['peaked', 'bimodal', 'flat']:
    z = make_distribution(shape)
    row = [mass_captured(z, k) for k in [10, 50, 100]]
    print(f"{shape:<10} | {row[0]:>10.3f} | {row[1]:>10.3f} | {row[2]:>11.3f}")

rng = np.random.RandomState(42)
z = make_distribution('flat')
samples = [top_k_sample(z, k=10, rng=rng) for _ in range(10)]
print(f"\nflat, k=10 samples: {samples}")

print("\nObservations:")
print("- Peaked: k=10 already captures 90% of the mass; k=50 or k=100 add almost nothing.")
print("- Flat: even k=100 (half the vocabulary) misses 15% of the real mass.")
print("- A fixed k cannot distinguish these cases; top-p (next section) adapts automatically.")
```
</RunnableCode>

The mass table makes the structural limitation from the previous section concrete: on the peaked distribution, $k = 50$ and $k = 100$ buy almost nothing over $k = 10$, forty or ninety extra kept tokens for a rounding error's worth of probability mass. On the flat distribution, even $k = 100$, half the vocabulary, still leaves $15\%$ of the real mass outside the kept set. Same $k$, opposite failure mode. Top-p fixes this by truncating on cumulative mass instead of rank.
...
Nucleus truncates on *cumulative* mass; min-p truncates on mass *relative to the top token*. Keep every token whose probability is at least $p_{\min}$ times the top token's probability, renormalize, sample:

<Equation label="19.minp">
$$\text{MinP}_{p_{\min}}(z) = \left\{i \,:\, p_i \geq p_{\min} \cdot \max_j p_j \right\}$$
</Equation>

The motivation (Nguyen et al., 2024) is that top-p's threshold is oblivious to *how* peaked the distribution is: at a fixed $p = 0.95$, a distribution with one dominant token and a distribution with two nearly-tied dominant tokens can produce very different-feeling nucleuses relative to their own shape. Min-p ties the threshold directly to the top token's confidence instead: when the model is very sure ($\max_j p_j$ is large), the bar for inclusion is high and few tokens survive; when the model is unsure ($\max_j p_j$ is small), the bar drops and more tokens survive, roughly proportional to how flat the whole distribution already is.
...
def min_p_sample(logits, p_min=0.1, T=1.0, rng=None):
    """Min-p truncation + sampling: keep tokens with p_i >= p_min * max_j p_j."""
    if rng is None:
        rng = np.random
    z = logits / T
    probs = softmax(z)
    ...
    mask = np.where(keep, z, -np.inf)
    p_final = softmax(mask)
    return int(rng.choice(len(p_final), p=p_final)), int(keep.sum())
...
print(f"{'Shape':<10} | {'kept, p_min=0.5':>15} | {'kept, p_min=0.1':>15} | {'kept, p_min=0.02':>16}")
print("-" * 66)
rng = np.random.RandomState(0)
for shape in ['peaked', 'bimodal', 'flat']:
    z = make_distribution(shape)
    counts = [min_p_sample(z, p_min=pm, rng=rng)[1] for pm in [0.5, 0.1, 0.02]]
    print(f"{shape:<10} | {counts[0]:>15} | {counts[1]:>15} | {counts[2]:>16}")

print("\nObservations:")
print("- Peaked: stays at 1 kept token even at a loose threshold (p_min=0.02);")
print("  nothing else comes within 2% of the top token's probability.")
print("- Flat: kept-set size explodes as p_min loosens (17 -> 98 -> 190 of 200);")
print("  when every token is nearly as likely as the max, min-p barely truncates.")
```
</RunnableCode>

Typical values are $p_{\min} = 0.05$ to $0.1$; below that min-p barely truncates anything, above $0.2$ it starts behaving like a much stricter top-p. Min-p is cheaper than top-p (no sort or cumulative sum needed, just a max and a threshold comparison) and composes cleanly with temperature, since raising $T$ flattens the whole distribution including the max, which loosens the min-p bar automatically. It is gaining adoption in open-source serving stacks but has not displaced nucleus as the default anywhere widely deployed.

Cumulative mass (top-p) and peak-relative mass (min-p) are not the only adaptive criteria in the literature. **Typical sampling** (Meister et al., 2022, [arxiv.org/abs/2202.00666](https://arxiv.org/abs/2202.00666)) truncates on *information content* instead: it keeps tokens whose surprisal $-\log p_i$ is closest to the distribution's entropy, on the reasoning that natural text tends to sit near the "typical" information rate at each step rather than at the extremes (neither the most predictable token nor the most surprising one). It never displaced nucleus sampling as a production default, but it is worth knowing as a third answer to "where do you cut the distribution": by rank (top-k), by cumulative mass (top-p), by distance from the top (min-p), or by distance from typical information content (typical sampling).
...
Beam search and greedy share a weakness: both pick tokens by likelihood alone, and likelihood-only decoding degenerates into repetition (that is the whole Holtzman finding that opened this chapter's case for nucleus sampling). Stochastic sampling avoids degeneration by adding randomness. **Contrastive search** (Su et al., $2022$, [arxiv.org/abs/2202.06417](https://arxiv.org/abs/2202.06417)) takes a different route: stay deterministic, but change the *scoring function* so that repetition is penalized directly at the representation level, not fixed by injecting noise.

At each step, take the $k$ highest-probability candidate tokens under the model (a small top-k shortlist, not the full vocabulary). For each candidate $v$, run it through the model far enough to get its hidden-state representation $h_v$, and score it by a weighted combination of two terms: how much the model favors it, and how similar its representation is to everything already generated.

<Equation label="19.contrastive-search">
$$x_t = \argmax_{v \in V^{(k)}} \Big[ (1 - \alpha) \, p_\theta(v \mid x_{<t}) \;-\; \alpha \max_{1 \le j < t} \text{sim}(h_v, h_{x_j}) \Big]$$
</Equation>

The first term, model confidence, is the same signal greedy decoding uses. The second term, the **degeneration penalty**, is the maximum cosine similarity between the candidate's representation and every previous token's representation, high when the candidate would repeat something the model already said in a similar way, low when it would say something new. $\alpha \in [0, 1]$ trades the two off; $\alpha = 0$ is plain greedy over the shortlist, and larger $\alpha$ pushes harder against repetition. Because the whole thing is a $\argmax$, decoding stays deterministic, reproducible, no random seed required, while still avoiding the degenerate loops that sink beam search and greedy.

<Callout type="note">
Contrastive search needs one thing greedy and nucleus sampling don't: access to hidden states during decoding, not just logits, to compute the similarity term. That is a real implementation cost (an extra forward-pass readout per candidate in the shortlist) which is part of why nucleus sampling, cheap and logits-only, remains the more common production default even where contrastive search would produce comparable or better text.
</Callout>
...
<Callout type="warning">
Constrained decoding guarantees valid *structure*; it does not make the model smarter about *content*. The mask only removes tokens that would break the grammar, it never adds knowledge the model doesn't have. If the model wasn't going to produce a sensible answer in the first place, an unmasked run would drift into rambling or a wrong-but-fluent answer; a masked run drifts into the same wrong content, just forced into valid JSON along the way, a value field with a plausible-looking but fabricated hallucination, or a key filled with the nearest legal token even though nothing in the vocabulary was actually a good fit at that position. The FSM enforces the grammar, not the correctness of what fills it. Constrained decoding is a guarantee about parseability, not about truth.
</Callout>
...
#
# # Expected sizes: peaked [1, 2, 9], bimodal [1, 19, 28], flat [8, 29, 36]
# # Observations:
# # - Peaked: nucleus stays tiny through p=0.9 (the dominant token alone covers
# #   ~90% of the mass); it only grows once the flat tail must be swept in for p=0.95
# # - Bimodal: the top token alone already clears 50% of the mass at p=0.5;
# #   reaching p=0.9-0.95 pulls in the second peak plus much of the tail
# # - Flat: nucleus is large at every p; no single token dominates
# # This is why top-p is the modern default: it adapts to distribution shape.
`}
  packages={["numpy"]}
/>
